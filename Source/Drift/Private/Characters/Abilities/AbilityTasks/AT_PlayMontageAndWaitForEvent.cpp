...
#include "Characters/Abilities/AbilityTasks/AT_PlayMontageAndWaitForEvent.h"
...
UDriftAbilitySystemComponent* UAT_PlayMontageAndWaitForEvent::GetTargetASC()
{
	return Cast<UDriftAbilitySystemComponent>(AbilitySystemComponent);
}
...
void UAT_PlayMontageAndWaitForEvent::Activate()
{
	...
	UDriftAbilitySystemComponent* DriftAbilitySystemComponent = GetTargetASC();

	if (DriftAbilitySystemComponent)
	{
		...
		if (AnimInstance != nullptr)
		{
			...
			EventHandle = DriftAbilitySystemComponent->AddGameplayEventTagContainerDelegate(EventTags, FGameplayEventTagMulticastDelegate::FDelegate::CreateUObject(this, &UAT_PlayMontageAndWaitForEvent::OnGameplayEvent));

			if (DriftAbilitySystemComponent->PlayMontage(Ability, Ability->GetCurrentActivationInfo(), MontageToPlay, Rate, StartSection) > 0.f)
			{
...
void UAT_PlayMontageAndWaitForEvent::OnDestroy(bool AbilityEnded)
{
	...
	if (DriftAbilitySystemComponent)
	{
		DriftAbilitySystemComponent->RemoveGameplayEventTagContainerDelegate(EventTags, EventHandle);
	}
